 ...
 * 自定义JavaBeanDeserializer 主要服务于没有默认constructor方法的对象
 */
public class JavaBeanDeserializerEx implements ObjectDeserializer {
    ...
    public JavaBeanDeserializerEx(ParserConfig config, Class<?> clazz) {
        this(config, clazz, clazz);
    }

    public JavaBeanDeserializerEx(ParserConfig config, Class<?> clazz, Type type) {
        this.clazz = clazz;

        beanInfo = DeserializeBeanInfoEx.computeSetters(clazz, type);
    ...
    public <T> T deserialze(DefaultJSONParser parser, Type type, Object fieldName, Object object) {
        ...
        try {
            ...
            if (lexer.token() == JSONToken.LBRACKET && lexer.isEnabled(Feature.SupportArrayToBean)) {
                return (T)deserialzeArrayMapping(parser, type, fieldName, object);
            }
